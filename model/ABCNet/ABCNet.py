...
from einops import rearrange #按给出的模式(注释)重组张量
...
# UCDC:U-shaped Convolution-Dilated Convolution
...
class ConvAttention(nn.Module):
    def __init__(self, in_dim, in_feature, out_feature):
        ...
        # nn.Parameter:将一个不可训练的tensor转换成可以训练的类型parameter
...
class ABCNet(nn.Module):
    def __init__(self, in_ch=1, out_ch=1, dim=64, ori_h=256, deep_supervision=True, mode='test',**kwargs):
    ...
    def forward(self, x):
        # print(x.shape)
        # x:(b, 3, w, h) (8 3 256 256)
        e1 = self.Conv1(x)

        # e1:(b, dim, w, h) -> filters = [dim, dim * 2, dim * 4, dim * 8, dim * 16]
        # CLFT
        ...
        if self.deep_supervision and self.mode=='train':
            outs = [d_s1.sigmoid(), d_s2.sigmoid(), d_s3.sigmoid(), d_s4.sigmoid(), d_s5.sigmoid(), out.sigmoid()]
        else:
            outs = out.sigmoid()
        return outs
