from model.holoconet.direction7 import *
from model.holoconet.direction5 import *
from model.holoconet.direction3 import *
...
class SEAM(nn.Module):
    ...
    def forward(self, x_high, x_low):
        # print(x_high.shape,x_low.shape)
        _, _, h, w = x_low.shape
        ...
        # print(x_low.shape,p_x_high.shape,x_high.shape)
        output = x_low * p_x_high + x_high
        return output


class HoLoCoNet(nn.Module):
    def __init__(self, layer_blocks=[4, 4, 4], channels=[16, 16, 32, 64],mode='test'):
    ...
    def forward(self, x):
        ...
        if self.mode=='test':
            if hei != wid:
                if hei < wid:
                    padding = (0, 0, 0, wid - hei)  # 左右不填充，上下填充 (wid - hei) 个 0
                else:
                    padding = (0, hei - wid, 0, 0)  # 上下不填充，左右填充 (hei - wid) 个 0
