...
parser.add_argument("--pth_dirs", default=["LimitIRSTD/HoLoCoNet_350.pth.tar"], nargs='+',
...
def test():
    ...
    file_path = 'datasets/LimitIRSTD/statistics.txt'  # 请替换为你的文件路径
    ...
    air_path = 'model_pth/Air+Un/HoLoCoNet_400_061921.pth.tar'
    land_path = 'model_pth/Land+Un/HoLoCoNet_390_061921.pth.tar'
    space_path = 'log/LimitIRSTD/HoLoCoNet_350.pth.tar'

    try:
        net_air_lwir.load_state_dict(torch.load(air_path)['state_dict'])
    except:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        net_air_lwir.load_state_dict(torch.load(air_path, map_location=device)['state_dict'])

    try:
        net_land_lwir.load_state_dict(torch.load(land_path)['state_dict'])
    except:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        net_land_lwir.load_state_dict(torch.load(land_path, map_location=device)['state_dict'])

    try:
        net_space_nir.load_state_dict(torch.load(space_path)['state_dict'])
    except:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        net_space_nir.load_state_dict(torch.load(space_path, map_location=device)['state_dict'])
    ...
    if opt.pth_dirs == None:
        for i in range(len(opt.model_names)):
            ...
            for dataset_name in opt.dataset_names:
                ...
                opt.pth_dir = opt.save_log + opt.dataset_name + '/' + opt.model_name + '_280.pth.tar'
                test()
            print('\n')
            opt.f.write('\n')
        opt.f.close()
