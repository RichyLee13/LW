...
from read_line import read_columns_to_dict
os.environ['KMP_DUPLICATE_LIB_OK'] = 'TRUE'
parser = argparse.ArgumentParser(description="PyTorch BasicIRSTD Inference without mask")
parser.add_argument("--model_names", default=['HoLoCoNet'], nargs='+',
                    help="model_name: 'ACM', 'ALCNet', 'DNANet', 'ISNet', 'UIUNet', 'RDIAN', 'ISTDU-Net', 'U-Net', 'RISTDnet'")
parser.add_argument("--pth_dirs", default=["WideIRSTD/HoLoCoNet_400.pth.tar"], nargs='+',
...
model_paths = {
    ('Air', 'LWIR'): 'model_pth/Air+Un/HoLoCoNet_400.pth.tar',
    ('Land', 'LWIR'): 'model_pth/Land+Un/HoLoCoNet_200.pth.tar',
    ('Space', 'NIR'): 'model_pth/Space+Un/HoLoCoNet_350.pth.tar',
    ('Space', 'SWIR'): 'model_pth/Space+Un/HoLoCoNet_350.pth.tar'
}
...
def test():
    ...
    file_path = 'datasets/WideIRSTD/statistics.txt'  # 请替换为你的文件路径
    ...
    # net.eval()
	# 创建四个模型实例
    net_air_lwir = Net(model_name=opt.model_name, mode='test').cuda()
    net_land_lwir = Net(model_name=opt.model_name, mode='test').cuda()
    net_space_nir = Net(model_name=opt.model_name, mode='test').cuda()

    try:
        net_air_lwir.load_state_dict(torch.load('model_pth/Air+Un/HoLoCoNet_400.pth.tar')['state_dict'])
    except:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        net_air_lwir.load_state_dict(torch.load('model_pth/Air+Un/HoLoCoNet_400.pth.tar', map_location=device)['state_dict'])

    try:
        net_land_lwir.load_state_dict(torch.load('model_pth/Land+Un/HoLoCoNet_400.pth.tar')['state_dict'])
    except:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        net_land_lwir.load_state_dict(torch.load('model_pth/Land+Un/HoLoCoNet_400.pth.tar', map_location=device)['state_dict'])

    try:
        net_space_nir.load_state_dict(torch.load('model_pth/Space+Un/HoLoCoNet_350.pth.tar')['state_dict'])
    except:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        net_space_nir.load_state_dict(torch.load('model_pth/Space+Un/HoLoCoNet_350.pth.tar', map_location=device)['state_dict'])


    with torch.no_grad():
        for idx_iter, (img, size, img_dir) in tqdm(enumerate(test_loader)):
            if img_dir[0] not in data_dict:
                net = net_air_lwir
            else:
                condition = data_dict[img_dir[0]]
                if condition == ('Air', 'LWIR'):
                    net = net_air_lwir
                elif condition == ('Land', 'LWIR'):
                    net = net_land_lwir
                elif condition == ('Space', 'NIR'):
                    net = net_space_nir
                elif condition == ('Space', 'SWIR'):
                    # net_space_swir = net_space_nir
                    net = net_space_nir
            net.eval()
